...
void common_operators(const Vec<T> &a, const Vec<T> &b, T c)
{
  ...
  Vec<T> x1; elem_mult_out(a,b,x1);
  cout << "elem_mult_out(a,b,x); x = " << x1 << endl;
  Vec<T> b2 = b; elem_mult_inplace(a,b2);
  ...
  a2 = a; a2.set_size(a2.size() + 3, true);
  ...
  a2 = a; a2.shift_left(c,2);
  cout << "a.shift_left(c,2) = " << a2 << endl;
  a2 = a; a2.shift_right(c);
  cout << "a.shift_right(c) = " << a2 << endl;
  a2 = a; a2.shift_left(b.mid(0,2));
