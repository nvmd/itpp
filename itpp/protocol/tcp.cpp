...
void TCP_Receiver::SendACK(bool sendConditionless)
{
   ...
   // ... oder wenn es das ACK für ein Out of Order Segment ist
   // ... oder (in der Realität), wenn ich auch was zu senden habe.
   ...
   if (sendConditionless || !fDelayedACK ||
      ...
      if (fAdvRcvNxt + fAdvRcvWnd + min(fBufferSize / 2, fMSS)
         ...
         // Die rechte Grenze des Empfängerfensters wird nur anders angezeigt
         // als beim letzten ACK, wenn sie sich seither um mindestens
         // min (BufferSize/ 2, MSS) geändert hat.
