  ...
  std::streamoff& audio_offset, std::streamoff& num_samples)
{
  ...
  if(data_size = 0xffffffff){
    ...
    num_samples = std::min(ns,(std::streamoff)data_size/(num_channels * sample_size));
  }
...
{
  uint32_t data_size = (uint32_t) std::min(num_samples * d->get_num_channels() * encoded_sample_size(d->get_encoding()), (std::streamoff)0xffffffff);
