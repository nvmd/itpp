...
namespace itpp { 
  ...
  void OFDM::set_parameters(const int inNfft, const int inNcp, const int inNupsample){
    ...
    norm_factor = sqrt(double(Nupsample*Nfft*Nfft)/(Nfft+Ncp));
    setup_done = true;
  }
  ...
  void OFDM::modulate(const cvec &input, cvec &output){
    ...
    for(int i=0; i<N; i++) {
      outtemp = ifft(input.mid(i*Nfft, Nfft),Nupsample*Nfft)*norm_factor;
  ...
  void OFDM::demodulate(const cvec& input, cvec &output){
    ...
    for(int i =0; i<N; i++) 
      output.replace_mid(Nfft*i, fft(input.mid(Nupsample*(i*(Nfft+Ncp)+Ncp), Nupsample*Nfft)).left(Nfft)/norm_factor);
  }
